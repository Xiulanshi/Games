...
@property(nonatomic) CGPoint previousPaddleLocation;
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size])
    {
        ...
        //sound actions
//        self.bounceSoundAction = [SKAction playSoundFileNamed:@"bounce.mp3" waitForCompletion:NO];
//        self.failSoundAction = [SKAction playSoundFileNamed:@"stab.mp3" waitForCompletion:NO];
    }
    return self;
}
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    else
    {
        ...
        NSLog(@"top = %f",self.manager.topCalibrationVal);
        NSLog(@"bottom = %f",self.manager.bottomCalibrationVal);
        
        //start playing if calibrated
        if(self.manager.isTopBottomCalibrated == true){
